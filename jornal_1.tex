...
Proposed hierarchical-deep model is a generative model of Deep Belief Network (DBN)[REF.]. Structure between each layer is shown in Fig. 4. each layer is considered as an Restricted Boltzmann Machine ($\bf{RBM}$)[REF.] except $\bf{\Gamma_K}$, $\bf{D^C}$, and $\bf{D^U}$. The MLN is trained by pseudo-log=likelihood as mention before, and RBM is trained by greedy layer-wise fashion [Ref.].
...
\begin{equation}
P(\theta^U_i|\bf{D^U})=\frac{1}{1+exp \left[\mu\left( -b_l-\sum_r d^U_r w_{lr}\right)\right]}
\end{equation}

where $\mu$ is penalty factor which decrease the probability while the inference is failed. 
