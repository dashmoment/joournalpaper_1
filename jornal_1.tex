...
P(\mathbf{D^C} = d^C_w  | \mathbf{D^{in}} = d^{in} , \mathbf{\Gamma}_{k \in {d^{in}}}) &\\\\
= P(d^{in} | \mathbf{MB}(d^{in}){P(d^{C}_w | \mathbf{MB}(d^{in}))}
\end{array}
\end{equation}
...
The common feature $\mathbf{\Gamma}_{k \in d^{in} \cap d^C_w}$ is further represented by co-Markov Blanket of $d^{in}$ and $d^C_w$, and the loss of mutual information can be further formulated by KL divergence [Ref.] as:

\begin{equation}
\begin{array}{ll}
\mathbf{argMin}D(P(\mathbf{MB}(d^{in},d^c_w)|d^in)||P(\mathbf{MB}(d^{in},d^c_w)|d^c_w))
\end{array}
\end{equation}
